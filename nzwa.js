...
async function starts() {
	...
	nzwa.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Escanea el código pero rapido'))
	})
	...
	await nzwa.connect({timeoutMs: 30*1000})
        ...
        console.log('=> Jhimer Bot v2!')
        lolcatjs.fromString('[Jhimer] Hola gracias por usar mi bot, espero te guste ;)')
	...
	nzwa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hola @${num.split('@')[0]}\n Bienvenido al grupo *${mdata.subject}* lee las reglas para que no haya malos entendidos`
			...
			const isCmd = body.startsWith(prefix)
                        ...
                        mess = {
							...
							only: {
								...
								daftarB: `──「 NO ESTÁS REGISTRADO 」──\nSuscribete al canal: https://youtube.com/channel/UCTERkWm7iM95hKZqvZfXicw\n Usa el comando: ${prefix}register para registrarte\nEjemplo: ${prefix}register Jhimer|20`,
                                }
                        }
					const apakah = ['Si','Nel']
        	        const bisakah = ['Ahorita','No puedo']
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				nzwa.updatePresence(from, Presence.composing) 
					...
					teks += `  Total : ${groupMembers.length}\n`
					for (let mem of groupMembers) {
						teks += `Jhimer 👤 ${mem.jid.split('@')[0]}@c.us\n`
						members_id.push(mem.jid)
					}
				...
				case 'alay':
                    ...
                    nzwa.sendMessage(from, hasil, image, {quoted: mek, caption: 'Jhimerrr V2'})
                    await limitAdd(sender)
                    break
                     case 'leave':
                    if (!isGroup) return reply(mess.only.group)
           ...
           break
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('No se pudo agregar porque es privado')
					}
					break
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						...
						reply('digite 1 para activar, 0 para desativar o recurso')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
				...
				case 'husbu2':
                 ...
                 break
					...
					nzwa.sendMessage(from, nye, image, { caption: 'Jhimer V2', quoted: mek })
				                      await limitAdd(sender)
				         break
					case 'goku':
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('Y el texto? Ejemplo ${prefix}phlogo Jhimer|Bot')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                ...
                                                buffer = await getBuffer(res.result)
                                                nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jhimer V2'})
                                                await limitAdd(sender)
                                        } catch (e) {
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❌ *ERROR* ❌')
				        ...
				        uptime = process.uptime()
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}lionlogo Jhimer|Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
					...
					break*/
                                ...
                                case 'burnpaper':
										...
										case 'coffee':
											var gh = body.slice(7)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}coffee JhimerBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
											anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=coffee&text=${teks1}&apikey=BotWeA`, {method: 'get'})
											buffer = await getBuffer(anu.result)
											client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jhimer V2'})
											await limitAdd(sender)
											break
									case 'lovepaper':
											var gh = body.slice(10)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}lovepaper JhimerBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'woodblock':
											var gh = body.slice(10)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}woodblock JhimerBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'qowheart':
											var gh = body.slice(9)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}qowheart JhimerBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'mutgrass':
											var gh = body.slice(9)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}mutgrass JhimerBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'undergocean':
											var gh = body.slice(12)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}undergocean JhimerBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
					...
					break
                                ...
                                case 'r':
                                    if (isRegister) return  reply(ind.rediregis())
                                        ...
                                        if (umurUser > 50) return reply(`La edad maxima es de 50 años`)
